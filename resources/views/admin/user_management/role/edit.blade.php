...
@section('title', 'Edit Role - AdminPanel')
...
@section('content')
    ...
    <h2 class="text-2xl font-bold text-slate-800">Edit Role</h2>
    <div class="md:p-8">
        <form action="/admin/user-management/role/{{ $role->id }}" method="POST" class="space-y-6" id="role-form">
            @csrf
            @method('PUT')
            <div>
            ...
            <div>
                ...
                <textarea
                    ...
                    rows="4">{{ old('deskripsi', $role->deskripsi) }}</textarea>
                @error('deskripsi')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
