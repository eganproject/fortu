...
@section('content')
    ...
    <div x-data="{ image1Preview: null, image2Preview: null }" class="bg-white/70 backdrop-blur-lg p-6 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.1)]">
            ...
            <form action="/admin/web-preferences/kategori/{{ $kategori->id }}" method="POST" class="space-y-6"
                ...
                <div>
                    ...
                    <select id="layout" name="layout"
                        class="bg-white border border-slate-300 text-slate-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                        required>
                        <option value="2" {{ old('layout', $kategori->layout) == '2' ? 'selected' : '' }}>2 Layout
                        </option>
                        <option value="3" {{ old('layout', $kategori->layout) == '3' ? 'selected' : '' }}>3 Layout
                        </option>
                    </select>
                    @error('layout')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
